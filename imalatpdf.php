...
$pdf->SetTitle('İmalat: ' . $siparisNo);
$pdf->SetSubject('İmalat Formu');
...
$pdf->Cell(0, 10, 'İMALAT FORMU', 0, 1, 'C');
...
$pdf->SetFont('dejavusans', '', 10);
$pdf->Cell(50, 7, guvenli_deger($row, 'yilsiparis'), 0, 1);

$pdf->SetFont('dejavusans', 'B', 10);
$pdf->Cell(40, 7, 'Ad Soyad:', 0, 0);
$pdf->SetFont('dejavusans', '', 10);
$pdf->Cell(50, 7, guvenli_deger($row, 'isim') . ' ' . guvenli_deger($row, 'soyisim'), 0, 1);

$pdf->SetFont('dejavusans', 'B', 10);
$pdf->Cell(40, 7, 'Telefon:', 0, 0);
$pdf->SetFont('dejavusans', '', 10);
$pdf->Cell(50, 7, guvenli_deger($row, 'ceptel'), 0, 1);

$pdf->SetFont('dejavusans', 'B', 10);
$pdf->Cell(40, 7, 'Adres:', 0, 0);
$pdf->SetFont('dejavusans', '', 10);
$pdf->MultiCell(0, 7, guvenli_deger($row, 'adres') . ' - ' . guvenli_deger($row, 'semt'), 0, 'L');

$pdf->SetFont('dejavusans', 'B', 10);
$pdf->Cell(40, 7, 'Ürün:', 0, 0);
$pdf->SetFont('dejavusans', '', 10);
$pdf->Cell(50, 7, guvenli_deger($row, 'urun'), 0, 1);

$pdf->SetFont('dejavusans', 'B', 10);
$pdf->Cell(40, 7, 'Açıklama:', 0, 0);
$pdf->SetFont('dejavusans', '', 10);
$pdf->MultiCell(0, 7, strip_tags(guvenli_deger($row, 'aciklama')), 0, 'L');

// Tarih bilgileri
$pdf->SetFont('dejavusans', 'B', 10);
$pdf->Cell(40, 7, 'Sipariş Tarihi:', 0, 0);
$pdf->SetFont('dejavusans', '', 10);
$pdf->Cell(50, 7, guvenli_deger($row, 'starih') . ' ' . guvenli_deger($row, 's_time'), 0, 1);

$pdf->SetFont('dejavusans', 'B', 10);
$pdf->Cell(40, 7, 'Teslim Tarihi:', 0, 0);
$pdf->SetFont('dejavusans', '', 10);
$pdf->Cell(50, 7, guvenli_deger($row, 'ttarih2') . ' ' . guvenli_deger($row, 'e_time'), 0, 1);

// Notlar
if (!empty(guvenli_deger($row, 'nt'))) {
    $pdf->SetFont('dejavusans', 'B', 10);
    $pdf->Cell(40, 7, 'Notlar:', 0, 0);
    $pdf->SetFont('dejavusans', '', 10);
    $pdf->MultiCell(0, 7, strip_tags(guvenli_deger($row, 'nt')), 0, 'L');
}

// İmza alanı
$pdf->Ln(20);
$pdf->Cell(95, 10, 'İmalat Sorumlusu', 0, 0, 'C');
$pdf->Cell(95, 10, 'Satış Personeli', 0, 1, 'C');
$pdf->Ln(15);
$pdf->Cell(95, 10, '_________________', 0, 0, 'C');
$pdf->Cell(95, 10, guvenli_deger($row, 'satis'), 0, 1, 'C');
...
// PDF'i çıktıla
$pdf->Output('Imalat_' . $siparisNo . '.pdf', 'I');
?>